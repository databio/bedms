...
model_path="nn_model4.pth"
encoder_path="encoder_model4.pth"
...
class ModelTraining:
    def __init__(self, input_size_values, input_size_headers, hidden_size, output_size, learning_rate):
        self.model=BoWSTModel(input_size_values, input_size_headers, hidden_size, output_size)
        ...
        self.train_losses=[]
        self.val_losses=[]
    def train(self, X_train_bow_tensor, X_train_headers_tensor, y_train_tensor, X_val_bow_tensor, X_val_headers_tensor, y_val_tensor, num_epochs, batch_size, device='cpu'):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, len(X_train_bow_tensor), batch_size):
                input_values = X_train_bow_tensor[i:i + batch_size].to(device)
            ...
            train_loss = total_loss_train / len(X_train_bow_tensor)
            ...
            total_loss_val=0.0
            with torch.no_grad():
                y_score = self.model(X_val_bow_tensor.to(device), X_val_headers_tensor.to(device)).cpu().numpy()
                for i in range(0, len(X_val_bow_tensor), batch_size):
                    input_values = X_val_bow_tensor[i:i + batch_size].to(device)
            ...
            val_loss = total_loss_val / len(X_val_bow_tensor)
...
input_size_values = X_train_bow_tensor.shape[1]
...
trainer.train(X_train_bow_tensor, X_train_headers_tensor, y_train_tensor, X_val_bow_tensor, X_val_headers_tensor, y_val_tensor, num_epochs=10, batch_size=32, device='cpu')
