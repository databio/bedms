...
#NN model - bag of words encoding
...
class ModelTraining:
    def __init__(self,input_size_values, input_size_headers, hidden_size, output_size, learning_rate):
        self.model=BoWModel(input_size_values, input_size_headers, hidden_size, output_size)
    ...
    def train(self, X_train_bow_tensor, X_train_header_bow_tensor, y_train_tensor, X_val_bow_tensor, X_val_header_bow_tensor, y_val_tensor, num_epochs, batch_size, device='cpu'):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, len(X_train_bow_tensor), batch_size):
                ...
                input_headers=X_train_header_bow_tensor[i:i+batch_size].to(device)
            ...
            total_loss_val=0.0
            with torch.no_grad():
                y_score = self.model(X_val_bow_tensor.to(device), X_val_header_bow_tensor.to(device)).cpu().numpy()
                for i in range(0, len(X_val_bow_tensor), batch_size):
                    input_values = X_val_bow_tensor[i:i + batch_size].to(device)
                    input_headers = X_val_header_bow_tensor[i:i+batch_size].to(device)
