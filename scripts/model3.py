...
def data_split(df_values, df_headers):
    ...
    X_headers_train_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_headers_train
    ]
    X_headers_test_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_headers_test
    ]
    X_headers_val_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_headers_val
    ]
...
def to_tensor(
    ...
    X_headers_train_tensor = torch.tensor(
        X_headers_train_bow.toarray(), dtype=torch.float32
    )
    X_headers_test_tensor = torch.tensor(
        X_headers_test_bow.toarray(), dtype=torch.float32
    )
    X_headers_val_tensor = torch.tensor(
        X_headers_val_bow.toarray(), dtype=torch.float32
    )
