...
def encoding(
    ...
    flattened_list = [
        item for sublist in X_values_train_list_top for col in sublist for item in col
    ]
    ...
    def encode_data(X_values_list, X_headers_list, y_list, batch_size):
        (
            X_values_bow_tensor_list,
            X_values_st_tensor_list,
            X_headers_st_tensor_list,
            y_tensor_list,
        ) = ([], [], [], [])
        data_batches = lazy_loading(
            list(zip(X_values_list, X_headers_list, y_list)), batch_size
        )
        for data_batch in data_batches:
            for X_values, X_headers, y in data_batch:
                X_values_bow = [
                    vectorizer.transform(col).toarray().flatten() for col in X_values
                ]
                X_values_bow_tensor_list.append(
                    torch.tensor(X_values_bow, dtype=torch.float32)
                )

                X_values_st = [
                    sentence_encoder.encode(col, show_progress_bar=False)
                    for col in X_values
                ]
                averaged_X_values_st = [
                    get_top_k_average(
                        [
                            torch.tensor(embedding, dtype=torch.float32)
                            for embedding in col
                        ]
                    )
                    for col in X_values_st
                ]
                X_values_st_tensor_list.append(torch.stack(averaged_X_values_st))

                X_headers_st = [
                    sentence_encoder.encode(col, show_progress_bar=False)
                    for col in X_headers
                ]
                averaged_X_headers_st = [
                    get_top_k_average(
                        [
                            torch.tensor(embedding, dtype=torch.float32)
                            for embedding in col
    ...
    batch_size = 32
    (
        X_values_train_bow_tensor,
        X_values_train_st_tensor,
        X_headers_train_tensor,
        y_train_tensor,
    ) = encode_data(X_values_train_list, X_headers_train_list, y_train_list, batch_size)
    (
        X_values_test_bow_tensor,
        X_values_test_st_tensor,
        X_headers_test_tensor,
        y_test_tensor,
    ) = encode_data(X_values_test_list, X_headers_test_list, y_test_list, batch_size)
    (
        X_values_val_bow_tensor,
        X_values_val_st_tensor,
        X_headers_val_tensor,
        y_val_tensor,
    ) = encode_data(X_values_val_list, X_headers_val_list, y_val_list, batch_size)
...
def data_loader(
    ...
    """
    datasets = []
    for X_values_bow_tensor, X_values_st_tensor, X_headers_st_tensor, y_tensor in zip(
        X_values_bow_tensor_list,
        X_values_st_tensor_list,
        X_headers_st_tensor_list,
        y_tensor_list,
        ...
        dataset = TensorDataset(
            X_values_bow_tensor, X_values_st_tensor, X_headers_st_tensor, y_tensor
        )
...
@profile
def custom_train(
    model, criterion, optimizer, train_loader, val_loader, num_epochs, checkpoint_path
):
    train_accuracies = []
    val_accuracies = []
    train_losses = []
...
def test_model(model, criterion, test_loader):
    ...
    print(
        f"Test Loss: {test_loss:.4f}, Test Accuracy: {accuracy:.2f}%, Precision: {precision:.4f}, Recall: {recall:.4f}, F1-score: {f1:.4f}"
    )
...
def hyperparameter_tuning(trial):
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=learning_rate, weight_decay=l2_reg_lambda
    )
    criterion = nn.CrossEntropyLoss()

    batch_size = 32
    train_loader = data_loader(
        X_values_train_bow_tensor_list,
        X_values_train_st_tensor_list,
        X_headers_train_tensor_list,
        y_train_tensor_list,
        batch_size,
    )
    val_loader = data_loader(
        X_values_val_bow_tensor_list,
        X_values_val_st_tensor_list,
        X_headers_val_tensor_list,
        y_val_tensor_list,
        batch_size,
    )

    num_epochs = 20
    train_accuracies, val_accuracies, train_losses, val_losses = custom_train(
    ...
    print(
        f"Trial {trial.number}, hidden_size: {hidden_size}, dropout_prob: {dropout_prob}, learning_rate: {learning_rate}, mean_val_accuracy: {mean_val_accuracy:.4f}"
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        X_headers_train_tensor_list,
        y_train_tensor_list,
        X_values_test_bow_tensor_list,
        X_values_test_st_tensor_list,
        X_headers_test_tensor_list,
        y_test_tensor_list,
        X_values_val_bow_tensor_list,
        X_values_val_st_tensor_list,
        X_headers_val_tensor_list,
