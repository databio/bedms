...
# TODO change the input to the model as multiple tables.
...
def data_split(df_values, df_headers):
    ...
    df_headers_test = pd.read_csv("../data/dummy_1.tsv", sep="\t")
    df_values_test = pd.read_csv("../data/dummy_1_header.tsv", sep="\t")
    ...
    # forming a single bag of words
    X_values_train_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_values_train
    ]
    X_values_test_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_values_test
    ]
    X_values_val_strings = [
        " ".join([str(val) for val in sublist]) for sublist in X_values_val
    ]
...
def to_tensor(
    ...
    X_values_train_tensor = torch.tensor(
        X_values_train_bow.toarray(), dtype=torch.float32
    )
    X_values_test_tensor = torch.tensor(
        X_values_test_bow.toarray(), dtype=torch.float32
    )
...
def confusion_mat(all_labels, all_preds, combined_labels):
    ...
    plt.savefig("confusion_matrix_model4.jpg")
    plt.show()
...
def auc_roc_curve(fpr, tpr, roc_auc, output_size):
    ...
    plt.savefig("roc_curve_model4.jpg")
    plt.show()


if __name__ == "__main__":
    values_file_path = "../data/encode_metadata_05022024.tsv"
    headers_file_path = "../data/encode_headers_matched.tsv"
    ...
    model = BoWSTModel(
        input_size_values, input_size_headers, hidden_size, output_size, dropout_prob
    )
    ...
    output_file_path = "predictions_model4.json"
    json_output(
        all_labels, all_preds, label_encoder, outputs, num_categories, output_file_path
    )
