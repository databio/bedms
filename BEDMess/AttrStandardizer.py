...
def standardize_attr_names(csv_file, schema):
    ...
    X_headers_embeddings_tensor, X_values_embeddings_tensor, X_values_bow_tensor, label_encoder = \
    ...
    padded_data_values_tensor = torch.zeros(X_values_bow_tensor.shape[0], target_size_values)
    padded_data_headers_tensor = torch.zeros(X_headers_embeddings_tensor.shape[0], target_size_headers)
    padded_data_values_embeddings_tensor = torch.zeros(X_values_embeddings_tensor.shape[0], target_size_values_embeddings)

    padded_data_values_tensor[:, :X_values_bow_tensor.shape[1]] = X_values_bow_tensor
    padded_data_headers_tensor[:, :X_headers_embeddings_tensor.shape[1]] = X_headers_embeddings_tensor
    ...
    all_confidences = []
    with torch.no_grad():
        outputs = model(padded_data_values_tensor, padded_data_values_embeddings_tensor, padded_data_headers_tensor)
